...
function mysqld_fetchallfields($tablename){
	$fields = mysqld_selectall("DESCRIBE {$tablename}", array(), 'Field');
...
class PdoUtil {
	...
	public function __construct($cfg) {
		...
		if(!class_exists('PDO')) {
		    include WEB_ROOT . '/framework/engine/base/class/lib/pdo/PDO.class.php';
		    
		}
		$options = array();
		if($cfg['is_preconnect']==true)
		{
		    $options=array(PDO::ATTR_PERSISTENT => true);
		}
		$mysqlurl = "mysql:dbname={$cfg['database']};host={$cfg['host']};port={$cfg['port']}";
		try { 
		    $this->dbo = new PDO($mysqlurl, $cfg['username'], $cfg['password'], $options);
...
function pdo_run($sql) {
	return mysqld_batch($sql);
}


function pdo_fieldexists($tablename, $fieldname = '') {
	return mysqld_fieldexists($tablename, $fieldname);
}


function pdo_indexexists($tablename, $indexname = '') {
	return mysqld_indexexists($tablename, $indexname);
}


function pdo_fetchallfields($tablename){
	return mysqld_fetchallfields($tablename);
}